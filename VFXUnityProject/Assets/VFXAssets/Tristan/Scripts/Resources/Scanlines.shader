Shader "Hidden/Shader/Scanlines"
{
    Properties
    {
        ...
        _Strength("Scanline Strength", float) = 0.5
        _Size("Scanline Size", float) = 0.5
    }
    ...
    float4 CustomPostProcess(Varyings input) : SV_Target
    {
        ...
        float2 scanlineUV = input.texcoord * (_ScreenParams.y / _Size);

        float3 lineColor = tex2D(_ScanlineTex, scanlineUV);

        // Apply greyscale effect
        float3 color = lerp(sourceColor, sourceColor * lineColor, _Strength);//lerp(sourceColor, Luminance(sourceColor), _Intensity);
